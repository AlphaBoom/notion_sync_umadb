...
def _createTitleProperty(text: str) -> Property:
    return Property(title=[RichText(text=Text(content=text))])


def _createRichTextProperty(text: str) -> Property:
    return Property(rich_text=[RichText(text=Text(content=text))])
...
class SkillDatabasePage(DatabasePage):
        ...
        self.properties = {
            ...
            "技能稀有度": Property(
                select=PropertySelect(
                    options=[
                        SelectOptions(
                            name=_rarity_mapping[SkillRarity.Normal],
                            color=ColorType.brown,
                        ),
                        SelectOptions(
                            name=_rarity_mapping[SkillRarity.Rare],
                            color=ColorType.yellow,
                        ),
                        SelectOptions(
                            name=_rarity_mapping[SkillRarity.Unique],
                            color=ColorType.pink,
                        ),
                        SelectOptions(
                            name=_rarity_mapping[SkillRarity.Upgrade],
    ...
    def createDatabase(self, database_name, parent_page_id: str) -> Database:
        return createDatabase(
            self._generateCreateDatabaseRequest(database_name, parent_page_id)
        )
    ...
    def filterNewSkill(
        self, skill_list: list[Skill], database_id: str
    ) -> tuple[list[Skill], list[tuple[Skill, Page]]]:
    ...
    def getPageInNotionDatabase(self, database_id, skill_id) -> Page:
        notion_list = queryDatabase(
            database_id, filter={"property": "id", "rich_text": {"equals": skill_id}}
        )
        if notion_list and notion_list.results:
            return notion_list.results[0]
        return None

    def _generateCreateDatabaseRequest(
        self, database_name, parent_page_id: str
...
class SkillDetailPage(DetailPage):
    def __init__(self, skill_icon_mapping=None, update_block=True) -> None:
        ...
        self._skill_effect_type_mapping = {item.value: item.name for item in SkillType}
        self._skill_effect_value_type_mapping = {
            item.value: item.name for item in SkillValueType
        }
    ...
    def _appendSkillInfo(self, skillData: SkillData, output: list):
        output.append(
            Block(
                paragraph=Paragraph(
                    rich_text=[
                        RichText(
                            text=Text(content="前置条件："),
                            annotations=Annotation(bold=True, color=ColorType.purple),
                        ),
                        RichText(text=Text(content=skillData.precondition)),
                    ]
                )
            )
        )
        output.append(
            Block(
                paragraph=Paragraph(
                    rich_text=[
                        RichText(
                            text=Text(content="发动条件："),
                            annotations=Annotation(bold=True, color=ColorType.purple),
                        ),
                        RichText(text=Text(content=skillData.condition)),
                    ]
                )
            )
        )
        output.append(
            Block(heading_3=Heading3(rich_text=[RichText(text=Text(content="效果信息"))]))
        )
        for effect in skillData.effectList:
            output.append(
                Block(
                    paragraph=Paragraph(
                        rich_text=[
                            RichText(
                                text=Text(
                                    content=self._skillEffectTypeFormat(effect.type)
                                    + "："
                                ),
                            ...
                            RichText(
                                text=Text(
                                    content=self._skillEffectValueFormat(
                                        effect.type, effect.value
                                    )
                                )
                            ),
                            RichText(
                                text=Text(content="  数值使用方式："),
                                annotations=Annotation(bold=True, color=ColorType.blue),
                            ),
                            RichText(
                                text=Text(
                                    content=self._skillEffectValueTypeFormat(
                                        effect.value_type
                                    )
                                )
                            ),
        ...
        output.append(
            Block(
                paragraph=Paragraph(
                    rich_text=[
                        RichText(
                            text=Text(content="持续时间："),
                            annotations=Annotation(bold=True, color=ColorType.purple),
                        ),
                        RichText(
                            text=Text(
                                content=self._skillDurationFormat(skillData.duration)
                            )
                        ),
                    ]
                )
        ...
        output.append(
            Block(
                paragraph=Paragraph(
                    rich_text=[
                        RichText(
                            text=Text(content="冷却时间："),
                            annotations=Annotation(bold=True, color=ColorType.purple),
                        ),
                        RichText(
                            text=Text(
                                content=self._skillDurationFormat(skillData.cooldown)
                            )
                        ),
                    ]
                )
    ...
    def _createPageDetail(self, skill: Skill) -> list[Block]:
        result = []
        result.append(
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="基本信息"))]))
        )
        result.append(
            Block(
                paragraph=Paragraph(
        ...
        if skill.dataList:
            ...
            self._appendSkillInfo(skill.dataList[0], result)
        if len(skill.dataList) > 1:
            result.append(
                Block(
                    heading_2=Heading2(rich_text=[RichText(text=Text(content="发动条件2"))])
                )
            )
            self._appendSkillInfo(skill.dataList[1], result)
        result.append(
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="其他信息"))]))
        )
        return result
