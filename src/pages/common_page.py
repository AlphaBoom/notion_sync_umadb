...
class DatabasePage(ABC):
    ...
    def properties(self, value):
        if _IS_TRANSLATED not in value:
            value[_IS_TRANSLATED] = Property(checkbox={})
        self._properties = value

    @staticmethod
    def attachTranslateProperty(pageProperties, is_translated:bool):
        if _IS_TRANSLATED not in pageProperties:
            pageProperties[_IS_TRANSLATED] = Property(checkbox=is_translated)
        return pageProperties
    
    @staticmethod
    def isPageTranslated(page:Page):
        property = page.properties.get(_IS_TRANSLATED)
        if property:
...
class DetailPage:
    def __init__(self, update_block=True) -> None:
        self._update_block = update_block

    def getFileInMapping(self, id, mapping: StrMapping) -> File:
        if mapping is not None:
            file = File(
                type=FileType.external, external=ExternalFile(url=mapping.get(id))
            )
    ...
    def _checkRequireUpdateBlocks(
        self, blocks: list[Block], new_blocks: list[Block]
            ...
            if block.type in (
                ...
                if len(rich_text) == len(rich_text_new):
                    for i in range(len(rich_text)):
                        if (
                            rich_text[i].text
                            and rich_text_new[i].text
                            and rich_text[i].text.content
                            != rich_text_new[i].text.content
                        ):
                            return False
                    return True
                else:
