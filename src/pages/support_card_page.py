...
from src.utils.support_card_helper import (
    parseEffectRow,
    parseUniqueEffectRow,
    effectTypeToStr,
)
...
class SupportCardDatabasePage(DatabasePage):
    def createPropertiesForPage(
        support_card_id,
        support_card_name,
        support_card_rarity: SupportCardRarity,
        ...
        return {
            ...
            "支援卡稀有度": Property(
                select=SelectOptions(name=_rarity_name_mapping[support_card_rarity])
            ),
            "支援卡类型": Property(
                select=SelectOptions(name=_card_type_name_mapping[support_card_type])
            ),
        ...
        self.properties = {
            ...
            "支援卡稀有度": Property(
                select=PropertySelect(
                    options=[
                        SelectOptions(
                            name=_rarity_name_mapping[SupportCardRarity.R],
                            color=ColorType.gray,
                        ),
                        SelectOptions(
                            name=_rarity_name_mapping[SupportCardRarity.SR],
                            color=ColorType.yellow,
                        ),
                        SelectOptions(
                            name=_rarity_name_mapping[SupportCardRarity.SSR],
            ...
            "支援卡类型": Property(
                select=PropertySelect(
                    options=[
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Speed],
                            color=ColorType.blue,
                        ),
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Stamina],
                            color=ColorType.red,
                        ),
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Power],
                            color=ColorType.orange,
                        ),
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Guts],
                            color=ColorType.pink,
                        ),
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Wiz],
                            color=ColorType.green,
                        ),
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Friend],
                            color=ColorType.brown,
                        ),
                        SelectOptions(
                            name=_card_type_name_mapping[SupportCardType.Team],
                            color=ColorType.purple,
                        ),
                    ]
                )
...
def _totalRaceStatusUp(card: SupportCard):
    ...
    if SupportCardEffectType.RaceStatusUp in card.effect_row_dict:
        ret += parseEffectRow(card.effect_row_dict[SupportCardEffectType.RaceStatusUp])[
            -1
        ]
    return ret
...
class SupportCardDetailPage(DetailPage):
    def __init__(
        self, cover_mapping=None, icon_mapping=None, update_block=True
    ) -> None:
        ...
        try:
            ...
            createPage(
                CreatePageRequest(
                    ...
                    properties=DatabasePage.attachTranslateProperty( SupportCardDatabasePage.createPropertiesForPage(
                        card.id,
                        card.name,
                        card.rarity,
                        card.type,
                        _totalRaceStatusUp(card),
                    ), card.is_translated),
    ...
    def _createPageDetail(
        self, card: SupportCard, skill_page_mapping, mismatch
    ) -> list[Block]:
        block_list = []
        if card.original_name:
            block_list.append(
                Block(
                    heading_2=Heading2(rich_text=[RichText(text=Text(content="描述信息"))])
                )
            )
            block_list.append(
                Block(
        ...
        block_list.extend(
            self._createEventSkillList(card, skill_page_mapping, mismatch)
        )
        block_list.extend(
            self._createTrainSkillList(card, skill_page_mapping, mismatch)
        )
        block_list.extend(self._createEffectTable(card))
        return block_list

    def _createEventSkillList(
        self, card: SupportCard, skill_page_mapping, mismatch
    ) -> list[Block]:
        ret = [
            Block(
                heading_2=Heading2(rich_text=[RichText(text=Text(content="事件可取得技能"))])
            )
        ]
    ...
    def _createTrainSkillList(
        self, card: SupportCard, skill_page_mapping, mismatch
    ) -> list[Block]:
        ret = [
            Block(
                heading_2=Heading2(rich_text=[RichText(text=Text(content="训练可取得技能"))])
            )
        ]
    ...
    def _createEffectTable(self, card: SupportCard) -> list[Block]:
        ret = [
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="基础效果信息"))]))
        ]
        if card.unique_effect:
            ...
            ret += [
                Block(
                    heading_3=Heading3(
                        rich_text=[RichText(text=Text(content=f"固有效果描述({level})"))]
                    )
                ),
                Block(
                    paragraph=Paragraph(rich_text=[RichText(text=Text(content=des))])
                ),
        ...
        if card.effect_row_dict:
            ...
            for row in rows:
                block = Block(
                    table_row=TableRow(
                        cells=[[RichText(text=Text(content=cell))] for cell in row]
                    )
                )
