...
class CharacterCardDatabasePage(DatabasePage):
    def createPropertiesForPage(
        id, name, talent_speed, talent_stamina, talent_power, talent_guts, talent_wiz
    ):
        return {
            "角色名称": Property(title=[RichText(text=Text(content=name))]),
            "速度成长率": Property(number=talent_speed / 100),
            "耐力成长率": Property(number=talent_stamina / 100),
            "力量成长率": Property(number=talent_power / 100),
            "根性成长率": Property(number=talent_guts / 100),
            "智力成长率": Property(number=talent_wiz / 100),
            "id": Property(rich_text=[RichText(text=Text(content=id))]),
        }
    ...
    def filterNewCard(
        self, card_list: list[CharacterCard], database_id: str
    ) -> tuple[list[CharacterCard], list[tuple[CharacterCard, Page]]]:
...
class CharacterCardDetailPage(DetailPage):
    def __init__(
        self,
        cover_mapping=None,
        icon_mapping=None,
        local_skill_info=None,
        update_block=True,
    ) -> None:
    ...
    def createPageInDatabase(
        ...
        database_id,
        card: CharacterCard,
        skill_page_mapping: StrMapping,
        missmatch: Callable[[str], str],
    ):
        try:
            icon_file = self.getFileInMapping(card.id, self.icon_mapping)
            cover_file = self.getFileInMapping(card.id, self.cover_mapping)
            children_list = self._createPageDetail(card, skill_page_mapping, missmatch)
            createPage(
                CreatePageRequest(
                    parent=Parent(type=ParentType.DATABASE, database_id=database_id),
                    properties=DatabasePage.attachTranslateProperty(CharacterCardDatabasePage.createPropertiesForPage(
    ...
    def updatePageInDatabase(
        ...
        try:
            ...
            properties = DatabasePage.attachTranslateProperty(properties, card.is_translated)
            children_list = self._createPageDetail(card, skill_page_mapping, mismatch)
            self.updatePageAndBlocks(
                properties, page, children_list, icon_file, cover_file
            )
        except Exception as e:
            traceback.print_exc()
            self.failed_count += 1
    ...
    def _createPageDetail(
        ...
        block_list.extend(
            self._createInstristicSkillInfo(card, skill_page_mapping, mismatch)
        )
        block_list.extend(
            self._createUpgradeSkillInfo(card, skill_page_mapping, mismatch)
        )
        block_list.extend(self._createSkillInfo(card, skill_page_mapping, mismatch))
        return block_list
    ...
    def _createStatusTable(self, card: CharacterCard) -> List[Block]:
        ...
        ret = []
        ret.append(
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="基础状态"))]))
        )
    ...
    def _createGroundProperTable(self, card: CharacterCard) -> List[Block]:
        ret = []
        ret.append(
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="场地适性"))]))
        )
        if card.proper_set:
            column_count = 2
                    ...
                    table_row=TableRow(
                        cells=[
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.turf]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.turf]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.dirt]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.dirt]
                                    ),
                                )
                            ],
                        ]
    ...
    def _createDistanceProperTable(self, card: CharacterCard) -> List[Block]:
        ret = []
        ret.append(
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="距离适性"))]))
        )
        if card.proper_set:
            column_count = 4
            proper = list(card.proper_set.values())[0]
                    ...
                    table_row=TableRow(
                        cells=[
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.short]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.short]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.mile]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.mile]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.middle]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.middle]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.long]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.long]
                                    ),
                                )
                            ],
                        ]
    ...
    def _createRunningStyleProperTable(self, card: CharacterCard) -> List[Block]:
        ret = []
        ret.append(
            Block(heading_2=Heading2(rich_text=[RichText(text=Text(content="跑法适性"))]))
        )
        if card.proper_set:
            column_count = 4
            proper = list(card.proper_set.values())[0]
                    ...
                    table_row=TableRow(
                        cells=[
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.nige]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.nige]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.senko]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.senko]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.sashi]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.sashi]
                                    ),
                                )
                            ],
                            [
                                RichText(
                                    text=Text(
                                        content=_proper_alphabet_mapping[proper.oikomi]
                                    ),
                                    annotations=Annotation(
                                        color=_proper_color_mapping[proper.oikomi]
                                    ),
                                )
                            ],
                        ]
    ...
    def _createInstristicSkillInfo(
        self,
        card: CharacterCard,
        skill_page_mapping: StrMapping,
        mismatch: Callable[[str], str],
        ...
        ret = []
        ret.append(Block(heading_2=Heading2(rich_text=[RichText(text=Text("固有技能"))])))
        ...
        for rank in card.rairty_skill_set.keys():
            ...
            if page_id == None:
                if skill_id in self.local_skill_info:
                    print(
                        f"Skill {skill_id} not found from cloud, using local skill info"
                    )
    ...
    def _createUpgradeSkillInfo(
        ...
        ret = []
        ret.append(Block(heading_2=Heading2(rich_text=[RichText(text=Text("进化技能"))])))
        ...
        for skill_id_array in card.upgrade_skill_set.values():
            for skill_id in skill_id_array:
                ...
                if page_id == None:
                    if skill_id in self.local_skill_info:
                        print(
                            f"Skill {skill_id} not found from cloud, using local sill info"
                        )
                        skill = self.local_skill_info[skill_id]
                        skill_list.append(RichText(text=Text(content=skill.name)))
                    else:
                        print(f"Skill {skill_id} not found ,neither local and cloud")
                else:
                    skill_list.append(
    ...
    def _createSkillInfo(
        ...
        ret = []
        ret.append(Block(heading_2=Heading2(rich_text=[RichText(text=Text("技能"))])))
